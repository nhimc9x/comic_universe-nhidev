...
function TextBox({ genres, title, chapter, description }) {
  return (
    <div data-swiper-parallax="-400" className="flex-1 overflow-hidden bg-black/30 backdrop-blur-sm sms:p-8 p-4 rounded-lg select-none">
      <div data-swiper-parallax="-300" className="text-lg mb-2 line-clamp-2">Thể loại: {genres && formatGenres(genres)}</div>
      <div data-swiper-parallax="-200" className="text-2xl md:text-4xl font-bold line-clamp-2 text-shadow mb-2">{title}</div>
